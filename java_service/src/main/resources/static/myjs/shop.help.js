...
function getShopData() {
    $.ajax({
        ...
        success: function (back) {
            ...
            document.getElementById("shop_place").innerHTML = shopFrame(navigationHtml);
...
function shopFrame(navigationHtml) {
    var frame =
        "<div class='col-lg-3 col-md-3 col-sm-3 col-xs-3'>" +
        navigationHtml +
        "</div>" +
        "<div class='col-lg-9 col-md-9 col-sm-9 col-xs-9' id='shop_show_algorithm_place'>" +
        "<table class='datatable-default algorithm-table' id='algorithmTablePlace' style='border: '>" +
        "<thead><tr class='algorithm-table-tr0'><th></th></tr><tr class='algorithm-table-tr1'><th>" + shopSortModel() + "</th></tr>" +
        "</thead></table>" +
        "</div>";
    return frame;
}
...
function createAlgorithmTypeModel(navigationTypeList, navigationCount) {
    ...
    html += "<li class='list-group-item'><div class='radio-button'><label><input type='radio' checked='checked' name='radio' onchange='shopChooseType(\"all\",\"all\")'><i class='helper'></i>查看所有类别(" + navigationCount["all"] + ")</label></div>";
    ...
    for (var i = 0, len = navigationTypeList.length; i < len; i++) {
        ...
        for (var j = 0, len1 = navigationList.length; j < len1; j++) {
            if (j == 0) {
                html += "<li class='list-group-item'><div class='radio-button'><label><input type='radio'  name='radio' onchange='shopChooseType(\"" + navigationTypeList[i]["id"] + "\",\"parent\")'><i class='helper'></i>查看所有(" + navigationCount[navigationTypeList[i]["id"]] + ")</label></div>";
            }
            html += "<li class='list-group-item'><div class='radio-button'><label><input type='radio' name='radio' onchange='shopChooseType(\"" + navigationList[j]["id"] + "\",\"child\")'><i class='helper'></i>" +
...
function shopChooseType(navigationId, type) {
    switch (type) {
        case "all":
...
function shopSortModel() {
    var html =
        "<a href='javascript:void(0);' onclick='shopSort(0)'><div class='algorithm-sort-click' sort_type='desc' name='algorithm_sort'>综合 <i class='fa fa-sort-desc'></i></div></a>" +
        "<a href='javascript:void(0);' onclick='shopSort(1)'><div class='algorithm-sort' sort_type='none' name='algorithm_sort'>更新时间 <i class='fa fa-sort'></i></div></a>" +
        "<a href='javascript:void(0);' onclick='shopSort(2)'><div class='algorithm-sort' sort_type='none' name='algorithm_sort'>浏览量 <i class='fa fa-sort'></i></div></a>" +
        "<a href='javascript:void(0);' onclick='shopSort(3)'><div class='algorithm-sort' sort_type='none' name='algorithm_sort'>使用次数 <i class='fa fa-sort'></i></div></a>" +
        "<a href='javascript:void(0);' onclick='shopSort(4)'><div class='algorithm-sort' sort_type='none' name='algorithm_sort'>收藏次数 <i class='fa fa-sort'></i></div></a>";
...
function shopSortAlgorithm(key, type) {
    switch (key) {
        case "name":
            if (type == "asc") {
                algorithmList.sort(shopAlgorithmNameAsc);
            } else {
                algorithmList.sort(shopAlgorithmNameDesc);
            }
            break;
        case "time":
            if (type == "asc") {
                algorithmList.sort(shopAlgorithmTimeAsc);
            } else {
                algorithmList.sort(shopAlgorithmTimeDesc);
            }
            break;
        case "visit":
            if (type == "asc") {
                algorithmList.sort(shopAlgorithmVisitAsc);
            } else {
                algorithmList.sort(shopAlgorithmVisitDesc);
            }
            break;
        case "use":
            if (type == "asc") {
                algorithmList.sort(shopAlgorithmUseAsc);
            } else {
                algorithmList.sort(shopAlgorithmUseDesc);
            }
            break;
        case "cart":
            if (type == "asc") {
                algorithmList.sort(shopAlgorithmCartAsc);
            } else {
                algorithmList.sort(shopAlgorithmCartDesc);
            }
            break;
    }
}

function shopAlgorithmNameAsc(x, y) {
    return x.algorithmName.localeCompare(y.algorithmName);
}

function shopAlgorithmNameDesc(x, y) {
    return y.algorithmName.localeCompare(x.algorithmName);
}

function shopAlgorithmTimeAsc(x, y) {
    return x.uploadTime.localeCompare(y.uploadTime);
}

function shopAlgorithmTimeDesc(x, y) {
    return y.uploadTime.localeCompare(x.uploadTime);
}

function shopAlgorithmVisitAsc(x, y) {
    return x.visitCount - y.visitCount;
}

function shopAlgorithmVisitDesc(x, y) {
    return y.visitCount - x.visitCount;
}

function shopAlgorithmUseAsc(x, y) {
    return x.useCount - y.useCount;
}

function shopAlgorithmUseDesc(x, y) {
    return y.useCount - x.useCount;
}

function shopAlgorithmCartAsc(x, y) {
    return x.cartCount - y.cartCount;
}

function shopAlgorithmCartDesc(x, y) {
...
//         case "comprehensive":
//             if (type == "asc") {
//                 return "algorithm-comprehensive-asc";
//             } else {
//                 return "algorithm-comprehensive-desc";
//             }
//             break;
//         case "time":
//             if (type == "asc") {
...
//         case "visit":
//             if (type == "asc") {
//                 return "visit-count-asc";
//             } else {
//                 return "visit-count-desc";
//             }
//             break;
//         case "use":
//             if (type == "asc") {
//                 return "use-count-asc";
//             } else {
//                 return "use-count-desc";
//             }
//             break;
//         case "cart":
//             if (type == "asc") {
//                 return "cart-count-asc";
//             } else {
//                 return "cart-count-desc";
//             }
//             break;
//     }
// }
