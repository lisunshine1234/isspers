...
function userStateChange(obj) {
    $.ajax({
        async: true,
        url: "/guteam/admin/user/audit/nonLock",
        type: "POST",
        dataType: "json",
        data: {"userId": $(obj).attr("u_id"), "nonLock": $(obj).prop("checked")},
        beforeSend: function () {
            swal({
                html: "<i  class='fa fa-spin fa-spinner'></i>正在更新用户状态，请稍等...",
                allowOutsideClick: false,
                allowEscapeKey: false,
                showConfirmButton: false
            });
...
function userUpdateModal(user) {
    var userNonLock;
    if (user.nonLock == true) {
        userNonLock = "<input type='checkbox' checked u_id='" + user.id + "' onchange='userStateChange(this)'>";
    } else {
        userNonLock = "<input type='checkbox' u_id='" + user.id + "' onchange='userStateChange(this)'>";
    }
    var userInfo =
        "<label>用户名</label>" +
