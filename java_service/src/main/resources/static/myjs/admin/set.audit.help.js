...
function setListFormat(setList) {
    ...
    for (var i = 0, len = setList.length; i < len; i++) {
        ...
        list.push(["<strong>" + (i + 1) + "</strong>",
            ...
            setList[i].userName,
            state,
            "<a href='javascript:void(0);' style='color: #0b93d5' onclick='auditSetById(\"" + setList[i]["id"] + "\")'>操作</a>"
        ]);
    }
    return list;
}

function auditSetById(setId) {
    var set = setList.filter(function (e) {
        return e.id == setId;
    })[0];
    var setInfo;
    if (set.nonLock == true) {
        setInfo = "<form>" +
            ...
            "<input type='checkbox' checked id='setNonLock' onchange='setStateChange(this)'>" +
        ...
        setInfo = "<form>" +
            ...
            "<div class='form-group'>" +
            "<label class='switcher'>" +
            "<input type='checkbox' id='setNonLock' onchange='setStateChange(this)'>" +
            "<span class='switcher-indicator'></span>" +
            "</label>" +
            "<div class='form-group'>" +
            "<label for='message-text' class='control-label'>信息</label>" +
    ...
    var html =
        ...
        "               </button>" +
        "               <h4 class='modal-title'>数据集锁定</h4>" +
        "           </div>" +
        "           <div class='modal-body' >" +
        setInfo +
        "           </div>" +
        "           <div class='modal-footer'>" +
        "               <button type='button' class='btn btn-primary float-button-light' onclick='setChangeNonLock(\"" + set.id + "\")'>提交 </button>" +
...
function setChangeNonLock(setId) {
    var nonLock = $("#setNonLock").prop("checked");
    var message = $("#message-text");
    myValidata(nonLock, -1, -1, "为锁定状态", "setNonLock");
    ...
    if (validataInfo.length > 0) {
        ...
        $.ajax({
            async: true,
            url: "/guteam/admin/set/audit/nonLock",
            type: "POST",
            data: {
                "json": JSON.stringify({
                    "setId": setId, "nonLock": nonLock, "message": message.val()
                })
            },
            dataType: "json",
            beforeSend: function () {
