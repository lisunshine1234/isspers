...
function algorithmListFormat(algorithmList) {
    ...
    for (var i = 0, len = algorithmList.length; i < len; i++) {
        ...
        list.push(["<strong>" + (i + 1) + "</strong>",
            ...
            state,
            "<a href='javascript:void(0);' style='color: #0b93d5' onclick='auditAlgorithmById(\"" + algorithmList[i]["id"] + "\")'>操作</a>"
        ]);
    }
    return list;
}

function auditAlgorithmById(algorithmId) {
    var algorithm = algorithmList.filter(function (e) {
        return e.id == algorithmId;
    })[0];
    var algorithmInfo;
    if (algorithm.nonLock == true) {
        algorithmInfo = "<form>" +
            "<label>未锁定状态</label>" +
            "<div class='form-group'>" +
            "<label class='switcher'>" +
            "<input type='checkbox' checked id='algorithmNonLock' onchange='algorithmStateChange(this)'>" +
        ...
        algorithmInfo = "<form>" +
            ...
            "<div class='form-group'>" +
            "<label class='switcher'>" +
            "<input type='checkbox' id='algorithmNonLock' onchange='algorithmStateChange(this)'>" +
            "<span class='switcher-indicator'></span>" +
            "</label>" +
            "<div class='form-group'>" +
            "<label for='message-text' class='control-label'>信息</label>" +
    ...
    var html =
        ...
        "               </button>" +
        "               <h4 class='modal-title'>算法锁定</h4>" +
        "           </div>" +
        "           <div class='modal-body' >" +
        algorithmInfo +
        "           </div>" +
        "           <div class='modal-footer'>" +
        "               <button type='button' class='btn btn-primary float-button-light' onclick='algorithmChangeNonLock(\"" + algorithm.id + "\")'>提交 </button>" +
        "               <button type='button' class='btn btn-default float-button-light' data-dismiss='modal'>关闭</button>" +
        "           </div>" +
        "        </div>" +
        "    </div>" +
        "</div>";
    $("#algorithmModalPlace").html(html);
...
function getAlgorithmInfoById(algorithmId) {
    $.ajax({
        ...
        beforeSend: function () {
            swal({
                html: "<i  class='fa fa-spin fa-spinner'></i>正在提交数据，请稍等...",
                allowOutsideClick: false,
                allowEscapeKey: false,
                showConfirmButton: false
            })
...
function algorithmStateChange(obj) {
...
function algorithmChangeNonLock(algorithmId) {
    var nonLock = $("#algorithmNonLock").prop("checked");
    var message = $("#message-text");
    myValidata(nonLock, -1, -1, "为锁定状态", "algorithmNonLock");
    ...
    if (validataInfo.length > 0) {
        ...
        $.ajax({
            async: true,
            url: "/guteam/admin/algorithm/update/nonLock",
            type: "POST",
            data: {
                "json": JSON.stringify({
                    "algorithmId": algorithmId, "nonLock": nonLock, "message": message.val()
                })
            },
            dataType: "json",
            beforeSend: function () {
