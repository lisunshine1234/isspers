...
function auditJobById(jobId) {
    var job = jobList.filter(function (e) {
        return e.id == jobId;
    })[0];
    var jobInfo;
    if (job.nonLock == true) {
        jobInfo = "<form>" +
            ...
            "<input type='checkbox' checked id='jobNonLock' onchange='jobStateChange(this)'>" +
        ...
        jobInfo = "<form>" +
            ...
            "<div class='form-group'>" +
            "<label class='switcher'>" +
            "<input type='checkbox' id='jobNonLock' onchange='jobStateChange(this)'>" +
            "<span class='switcher-indicator'></span>" +
            "</label>" +
            "<div class='form-group'>" +
            "<label for='message-text' class='control-label'>信息</label>" +
    ...
    var html =
        "<div class='modal fade ' id='jobStateModal' >" +
        ...
        "               </button>" +
        "               <h4 class='modal-title'>任务锁定</h4>" +
        "           </div>" +
        "           <div class='modal-body' >" +
        jobInfo +
        "           </div>" +
        "           <div class='modal-footer'>" +
        "               <button type='button' class='btn btn-primary float-button-light' onclick='jobChangeNonLock(\"" + job.id + "\")'>提交 </button>" +
        "               <button type='button' class='btn btn-default float-button-light' data-dismiss='modal'>关闭</button>" +
        "           </div>" +
        "        </div>" +
        "    </div>" +
        "</div>";
    $("#jobStateModalPlace").html(html);
    $("#jobStateModal").modal('show');

}

function jobStateChange(obj) {
...
function jobChangeNonLock(jobId) {
    var nonLock = $("#jobNonLock").prop("checked");
    var message = $("#message-text");
    myValidata(nonLock, -1, -1, "为锁定状态", "jobNonLock");
    if ($("#message-text").attr("disabled") == null) {
        myValidata(message.val(), 1, 300, "信息", "message-text");
    }
    ...
    if (validataInfo.length > 0) {
        ...
        $.ajax({
            async: true,
            url: "/guteam/admin/job/audit/nonLock",
            type: "POST",
            data: {
                "json": JSON.stringify({
                    "jobId": jobId, "nonLock": nonLock, "message": message.val()
                })
            },
            dataType: "json",
            beforeSend: function () {
                swal({
                    html: "<i  class='fa fa-spin fa-spinner'></i>正在更改锁定状态，请稍等...",
                    allowOutsideClick: false,
                    allowEscapeKey: false,
                    showConfirmButton: false
                })
...
function jobListFormat(jobList) {
    ...
    for (var i = 0, len = jobList.length; i < len; i++) {
        ...
        list.push(["<strong>" + (i + 1) + "</strong>",
            ...
            jobList[i].userName,
            state,
            "<a href='javascript:void(0);' style='color: #0b93d5' onclick='auditJobById(\"" + jobList[i]["id"] + "\")'>操作</a>"
        ]);
    }
    return list;
}
    ...
    if ('WebSocket' in window) {
        $.ajax({
            ...
            success: function (back) {
                ...
                websocket = new WebSocket("ws://localhost/guteam/webSocket/job/run/async/" + jobId);
...
function runDisplayReview(back) {
    ...
    for (var i = 0, len1 = jobList.length; i < len1; i++) {
        // var temp = $.parseJSON(jobList[i]);
        var temp = jobList[i];
        ...
        th += "<th>算法#" + (i + 1) + "</th>";
        td[0] += "<td>" + job["jobName"] + "</td>";
        td[1] += "<td>" + job["jobEnvironment"] + "</td>";
        td[2] += "<td>" + job["jobEngine"] + "</td>";
        td[3] += "<td>" + job["jobSize"] + "</td>";
...
function myValidata(value, min_len, max_len, name, id) {
    var obj = $("#" + id);
    if (min_len < 0) {
        min_len = 0;
    }
